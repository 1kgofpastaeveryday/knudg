...
Publication has no per-card human approval step. Per [target-model.md](target-model.md), uploading to the backend is the decision to share, the GLM filter is the sole gate, and filter-pass publishes automatically; the prior signed-challenge / consent-surface apparatus was retired. Revoke remains the only way back out after publish and must always work.
