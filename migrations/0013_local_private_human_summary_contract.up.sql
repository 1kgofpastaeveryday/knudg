create or replace function knudg_private.local_private_human_summary_is_valid(row_value jsonb)
returns boolean
language sql
immutable
set search_path = pg_catalog, knudg_private, pg_temp
as $$
  select jsonb_typeof(row_value) = 'object'
    and not exists (
      select 1
      from jsonb_object_keys(row_value) as key
      where key not in ('content', 'redaction_summary')
    )
    and row_value ?& array['content','redaction_summary']
    and jsonb_typeof(row_value->'content') = 'string'
    and length(row_value->>'content') between 20 and 240
    and knudg_private.local_private_text_is_sanitized(row_value->>'content')
    and jsonb_typeof(row_value->'redaction_summary') = 'string'
    and length(row_value->>'redaction_summary') between 20 and 240
    and knudg_private.local_private_text_is_sanitized(row_value->>'redaction_summary');
$$;
...
as $$
  select jsonb_typeof(row_body) = 'object'
    ...
    and not exists (
      ...
      where key not in (
        'source_class','title','human_summary','problem_summary','solution_summary',
    ...
    and knudg_private.local_private_human_summary_is_valid(row_body->'human_summary')
